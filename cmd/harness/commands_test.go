...
// newTestSessionManager 构建带临时会话的测试 SessionManager（HARNESS_HOME 隔离 + 显式配置）。
func newTestSessionManager(t *testing.T) *SessionManager {
	...
	t.Cleanup(m.closeAll) // 关闭所有 open 会话的 writer（含 /switch 后 resume 的）
	return m
}
...
func TestHandleCommandEffort(t *testing.T) {
	...
	if err := m.handleCommand(replCommand{name: "effort", arg: "low"}); err != nil {
		t.Fatalf("/effort low: %v", err)
	}
	if got := m.active.State().ThinkingEffort; got != "low" {
...
func TestHandleCommandEffortInvalid(t *testing.T) {
	m := newTestSessionManager(t)
	err := m.handleCommand(replCommand{name: "effort", arg: "bogus"})
...
func TestHandleCommandModel(t *testing.T) {
	m := newTestSessionManager(t)
	if err := m.handleCommand(replCommand{name: "model", arg: "m"}); err != nil {
		t.Fatalf("/model m: %v", err)
	}
	if got := m.active.Model(); got != "m" {
		t.Errorf("active model: got %q", got)
	}
	// 新模型重置 effort 为模型默认（high）。
	if got := m.active.State().ThinkingEffort; got != "high" {
...
func TestHandleCommandModelInvalid(t *testing.T) {
	m := newTestSessionManager(t)
	err := m.handleCommand(replCommand{name: "model", arg: "nonexistent"})
...
func TestSessionManagerSwitch(t *testing.T) {
	...
	if err := m.switchTo(other.ID); err != nil {
		t.Fatalf("switchTo: %v", err)
	}
	if m.active.ID != other.ID {
	...
	if err := m.switchTo(firstID); err != nil {
		t.Fatalf("switchTo back: %v", err)
	}
	if m.active != m.open[firstID] {
		t.Fatal("active 未切回初始会话实例")
	}
}

// TestSessionManagerSwitchUnknown 验证切换不存在的会话报错。
func TestSessionManagerSwitchUnknown(t *testing.T) {
	m := newTestSessionManager(t)
	err := m.switchTo("nonexistent")
