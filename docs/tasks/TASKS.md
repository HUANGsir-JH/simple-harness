...
- **状态**：✅ 已完成（2026-08-14，ADR-043；分支 feat/tui-redesign 六阶段全绿）
...
| T1 | theme.go（token + 明暗探测 + 样式构建，迁移全局 style 变量）+ keymap.go（键位表驱动，行为零变化）；验收 = 零测试改动全绿 | ✅ 2026-08-14 |
| T2 | dialogs.go 统一 modal 框架 + 审批/ask/select/help 四实例迁移 | ✅ 2026-08-14 |
| T3 | cells.go + 布局视觉（header/footer/aux/composer）+ 右侧滚动条 + 鼠标文本选择状态机 + 回合/thinking 时长 | ✅ 2026-08-14 |
| T4 | tools.go 工具双形态 + 展开态完整 args + 工具耗时 + diff/代码高亮（chroma） | ✅ 2026-08-14 |
| T5 | render.go cell 渲染缓存（P1；P2 合帧未做，收益评估后置） | ✅ 2026-08-14 |
| T6 | 测试迁移收尾 + e2e 全绿 + 文档（PROGRESS/IMPLEMENTATION_PLAN）+ 人工实测清单（待用户实测） | ✅ 2026-08-14（人工实测待用户） |
