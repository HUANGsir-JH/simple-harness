...
func testChain() *middleware.Chain {
	return middleware.NewChain(ToolOutputMiddleware{})
}
...
func TestToolOutputMiddlewareTruncates(t *testing.T) {
	...
	core := func(ctx context.Context, rc *middleware.RuntimeContext, in middleware.ToolCallInput) error {
		conv.Add(messages.NewToolResultsMessage([]messages.ToolResultBlock{
	...
	if err := wrapped(context.Background(), rc, middleware.ToolCallInput{Calls: nil}); err != nil {
...
func TestToolOutputMiddlewareNilMessages(t *testing.T) {
	rc := middleware.NewRuntimeContext() // Messages nil
	chain := testChain()
	core := func(ctx context.Context, rc *middleware.RuntimeContext, in middleware.ToolCallInput) error {
		return nil
	}
	if err := chain.WrapToolCall(core)(context.Background(), rc, middleware.ToolCallInput{}); err != nil {
...
func TestToolOutputMiddlewareSkipsReadFile(t *testing.T) {
	...
	wrapped := chain.WrapToolCall(core)
	if err := wrapped(context.Background(), rc, middleware.ToolCallInput{Calls: calls}); err != nil {
		t.Fatalf("wrapped: %v", err)
	}
